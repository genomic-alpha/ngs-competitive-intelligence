...
const SequencerLandscape = ({ products }) => {
  const data = useMemo(() => {
    return products
      ...
      .map(p => {
        const v = VENDORS.find(v => v.key === p.vendor);
        return { name: p.name, share: p.share || 0, pricing: (p.pricing || 0) / 1000, vendor: v?.label || p.vendor, color: v?.color || '#6b7280', tier: p.tier, regulatory: p.regulatory };
      });
  }, [products]);
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <ScatterChart>
        <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
        <XAxis dataKey="pricing" stroke="#9ca3af" name="Price" unit="k" label={{ value: 'Run Cost ($k)', position: 'insideBottom', offset: -5, fill: '#9ca3af', fontSize: 11 }} />
        ...
        <Tooltip
          contentStyle={{ backgroundColor: '#1f2937', border: '1px solid #374151', borderRadius: '8px' }}
          formatter={(val, name) => [name === 'Price' ? `$${val}k` : `${val}%`, name]}
          labelFormatter={() => ''}
          content={({ active, payload }) => {
            ...
            return (
              <div className="bg-gray-800 border border-gray-600 rounded-lg p-3 text-sm">
                ...
                <p className="text-blue-400">Share: {d.share}% · Cost: ${d.pricing}k</p>
